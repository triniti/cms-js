...
import SearchSortEnum from '@triniti/schemas/triniti/dam/enums/SearchAssetsSort';
import {
  Button,
  Card,
  CardBody,
...
class SearchAssetsForm extends Component {
  constructor(props) {
    ...
    this.handleToggleAssetTypes = this.handleToggleAssetTypes.bind(this);
  ...
  handleToggleAssetTypes(assetType) {
    const { request } = this.state;
    ...
    this.setState({
      request: {
        ...request,
        types: !types.includes(assetType)
          ? [...types, assetType]
  ...
  render() {
    ...
    return (
      <Card className={isCollapsed ? 'sticky-top search-assets-form' : 'search-assets-form'}>
        <CardBody className="pb-1">
          <Form
            autoComplete="off"
            className="search-assets-form"
            onSubmit={(e) => {
              e.preventDefault();
            }}
          >
            <InputGroup>
              ...
              <InputGroupAddon addonType="append">
                <Button color="secondary" disabled={disabled} onClick={this.handleSearch}>
            ...
            <div className="d-inline-block mt-2">
              ...
              && schemas.node.map((item) => item.getId().getMessage()).map((assetType) => {
                const hasTypes = types ? types.includes(assetType) : null;
                return (
                  <Button
                    active={hasTypes}
                    color={hasTypes ? 'secondary' : 'light'}
                    id={`asset-button.${assetType}`}
                    key={`asset-button.${assetType}`}
                    name={assetType}
                    onClick={(e) => this.handleToggleAssetTypes(e.target.id.replace('asset-button.', ''))}
                    outline
                    size="sm"
                  >
                    { assetType.replace('-asset', '') }
                    <Icon
                      className="ml-1 mr-0 icon-no-events"
                      imgSrc={hasTypes ? 'minus-outline' : 'plus-outline'}
                      size="xs"
                    />
                  </Button>
                );
