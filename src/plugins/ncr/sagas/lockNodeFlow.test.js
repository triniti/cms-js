...
const action = {
  config,
  pbj: config.schemas.lockNode.createMessage().set('node_ref', NodeRef.fromNode(fakeNode)),
};

const verify = () => {};

const failureMessage = 'Lock Article failed: ';
const successMessage = 'Success! The Article was locked.';
const lockNodeFlowConfig = {
  expectedEvent: action.config.schemas.nodeLocked.getCurie().toString(),
  failureMessage,
...
test('Ncr:saga:lockNodeFlow', (t) => {
  const generator = lockNodeFlow(action);
  let next = generator.next();

  const actual = next.value;
  const expected = call(changeNodeFlow, lockNodeFlowConfig);
...
test('Ncr:saga:lockNodeFlow:changeNodeFlow[success]', (t) => {
  const generator = changeNodeFlow(lockNodeFlowConfig);
  let next = generator.next();
  ...
  const expectedEvent = action.config.schemas.nodeLocked.getCurie().toString();
  actual = next.value;
...
test('Ncr:saga:lockNodeFlow:changeNodeFlow[failure:waitForFlow]', (t) => {
  const generator = changeNodeFlow(lockNodeFlowConfig);
  let next = generator.next();
...
test('Ncr:saga:lockNodeFlow:changeNodeFlow[failure:thrown]', (t) => {
  const generator = changeNodeFlow(lockNodeFlowConfig);
  let next = generator.next();
