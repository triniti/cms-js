...
class SortableList extends React.Component {
  ...
  render() {
    ...
    let label = isMulti ? 'order pages' : 'selected page';
    if (readOnly) {
      label = isMulti ? 'related pages' : 'related page';
    }
      ...
      <FormGroup>
        ...
        <Row>
          <div ref={(el) => { this.sortableListRef = el; }} className="col">
            {fields.getAll().map((pageRef, i) => {
              const page = getNode(pageRef);
              if (!page) {
                return (
                  <Media key={pageRef} className="align-items-center sortable-articles mb-3">
                    <Media heading className="mb-0">
                      <strong>{`${pageRef}`}</strong> did not load or is not found
                    </Media>
                    {
                      !readOnly && (
              ...
              const link = pbjUrl(page, 'canonical');
                ...
                <Media key={pageRef} className="align-items-center sortable-articles mb-3">
                  ...
                  <Media body>
                    <Media heading className="mb-0">
                      ...
                      <small
                        className={`text-uppercase ml-2 status-copy status-${page.get('status')}`}
                      >
                        {`${page.get('status')}`}
                      </small>
                    </Media>
