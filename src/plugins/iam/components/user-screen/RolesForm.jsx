...
import grantRolesToUser from '@triniti/cms/plugins/iam/actions/grantRolesToUser';
import revokeRolesFromUser from '@triniti/cms/plugins/iam/actions/revokeRolesFromUser';
...
function RolesForm(props) {
  ...
  const canGrant = policy.isGranted('gdbots:iam:command:grant-roles-to-user') && !isEmpty(grants);
  const canRevoke = policy.isGranted('gdbots:iam:command:revoke-roles-from-user') && !isEmpty(revokes);

  const handleGrant = async () => {
    try {
      await progressIndicator.show(`Granting role(s) to ${node.get('title')}...`);
      await dispatch(grantRolesToUser(nodeRef, Object.keys(grants)));
      await refreshNode();
  ...
  const handleRevoke = async () => {
    try {
      ...
      await dispatch(revokeRolesFromUser(nodeRef, Object.keys(revokes)));
      await refreshNode();
      await progressIndicator.close();
