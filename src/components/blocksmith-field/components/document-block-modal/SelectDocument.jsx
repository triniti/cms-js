    ...
    <>
      <Form className="shadow-depth-2 sticky-top" onSubmit={handleSubmit} autoComplete="off">
        <InputGroup className="px-4 py-2">
          <Field name="q" type="search" component="input" className="form-control" placeholder="Search Document Assets" />
            ...
            <tbody>
              {response.get('nodes', []).map(node => {
                return (
                  <tr
                    key={`${node.get('_id')}`}
                    onClick={() => setSelectedDocumentNode(node)}
                    className={node.equals(selectedDocumentNode) ? 'bg-selected' : ''}
                  >
...
export default withRequest(withForm(SelectDocument), 'triniti:dam:request:search-assets-request', {
  channel: 'blocksmith',
  persist: true,
  initialData: {
