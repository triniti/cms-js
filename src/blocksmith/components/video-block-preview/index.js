...
function VideoBlockPreview(props) {
  ...
  const imageUrl = damUrl(block.get('poster_image_ref', node.get('poster_image_ref', node.get('image_ref'))), '16by9', 'sm');
  const status = node.get('status').getValue();
  const url = nodeUrl(node, 'view');
  const launchText = block.get('launch_text', node.get('launch_text'));
      ...
      {imageUrl && (
        <Col xs={2}>
          <a href={url} className="hover-box-shadow d-inline-block rounded-2" target="_blank">
            <Media src={imageUrl} className="rounded-2 ratio-16x9" alt="" width="100%" height="auto" object />
          </a>
        </Col>
      )}
      <Col>
        <Table borderless size="sm">
          ...
          {launchText && (
            <tr>
              ...
              <td className="w-100 text-break">{launchText}</td>
            </tr>
          )}
          <tr>
